...
namespace Graph {
	...
	// O(V(V + E)). В среднем время ~VE.
    // За один проход из вершины v мы за O(V+E) получаем множество вершин, достижимых из v.
    // За V проходов по всем вершинам мы получим за O(V(V+E)) множество достижимости для всех вершин.
